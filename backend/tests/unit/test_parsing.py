...
class TestParsing:
    ...
    def test_basic_quantity_name(self):
        ...
        result = parse_mtg_line(line)
        
        assert result is not None
        assert result["qty"] == 4  # CORRECTION: "qty" au lieu de "quantity"
        assert result["name"] == "Lightning Bolt"
        assert result.get("set") is None
